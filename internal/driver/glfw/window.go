...
func (w *window) Show() {
	async.EnsureMain(func() {
		...
		} else if w.fullScreen { // this does not work if called before viewport.Show()
...
func (d *gLDriver) createWindow(title string, decorate bool) fyne.Window {
	...
	ret = &window{title: title, decorate: decorate, driver: d, frame: newPresentGate(), mousePosUpdateProcessed: true}
