...
- c_glfw_bsd.go — omit duplicate null_joystick.c and obsolete Wayland protocol .c includes; common c_glfw.go and wl_init.c already include their implementations
- native_linbsd_wayland.go + native_linbsd_wayland_refyne.go + not_wayland.go — BSD defaults to X11; only include Wayland wrappers with the wayland tag (Linux defaults to both backends)
...
- glfw/src/wl_platform.h sizeCommitPending + glfw/src/wl_window.c + glfw/src/wl_refyne.c + glfw/src/egl_context.c — set in resizeFramebuffer, cleared after eglSwapBuffers: suppress bare wl_surface commits while staged size state (EGL buffer size, viewport destination, xdg window geometry) awaits a content-buffer swap. Some EGL WSIs still attach one buffer acquired before the initial mapped size was known; the refyne GLFW driver retires it once while mapping and paints the fresh buffer immediately. Live resizes use one painted swap so an undefined intermediate EGL buffer is never presented.
...
- glfw/src/egl_context.c + wl_window.c _glfwPrimePendingEGLResizeWayland — makeContextCurrentEGL applies the deferred wl_egl_window resize on BOTH sides of eglMakeCurrent: this covers Mesa/NVIDIA differences in when a pending resize is observed. It does not guarantee that a buffer acquired before the initial map is discarded; the driver-side framebuffer settling path handles that case once per mapping.
