...
- glfw/src/wl_window.c — lazy scaling viewport: created in fractionalScaleHandlePreferredScale only when the preferred scale is not 120 (1.0) and destroyed when it returns to 120, instead of upstream's unconditional creation. At scale 1.0 the viewport's presence lets the compositor scale content on any perceived buffer/destination disagreement during interactive resize; without it a mismatch can only lag. Also rounds fbWidth/fbHeight half-up ((w*num+60)/120) per fractional-scale-v1, where upstream truncates
