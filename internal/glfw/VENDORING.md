...
- glfw/src/egl_context.c + wl_window.c _glfwPrimePendingEGLResizeWayland — makeContextCurrentEGL applies the deferred wl_egl_window resize on BOTH sides of eglMakeCurrent: this covers Mesa/NVIDIA differences in when a pending resize is observed. It does not guarantee that an already-acquired back buffer is discarded; the driver-side framebuffer settling path handles that case.
