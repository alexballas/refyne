...
func TestRichTextMarkdown_CodeBlockScrolls(t *testing.T) {
	...
	assert.Less(t, min.Width, float32(200))
...
func TestRichTextMarkdown_Heading(t *testing.T) {
	...
	if text, ok := r.Segments[1].(*TextSegment); ok {
		...
		t.Error("Segment should be Paragraph")
	}
	if text, ok := r.Segments[2].(*TextSegment); ok {
		assert.Equal(t, "Head2", text.Text)
...
func TestRichTextMarkdown_HeadingWithHyperlink(t *testing.T) {
	...
	if link, ok := r.Segments[6].(*HyperlinkSegment); ok {
		...
		assert.Equal(t, RichTextStyleStrong.TextStyle, link.TextStyle)
	} else {
...
func TestRichTextMarkdown_TaskList(t *testing.T) {
	...
	assert.Len(t, r.Segments, 1)
	list, ok := r.Segments[0].(*ListSegment)
	if !ok {
		t.Fatal("Segment should be a List")
	}
	assert.Len(t, list.Items, 2)

	first := list.Items[0].(*ParagraphSegment).Texts[0].(*CheckBoxSegment)
	assert.True(t, first.Checked)
	assert.Equal(t, "done", first.Text)

	second := list.Items[1].(*ParagraphSegment).Texts[0].(*CheckBoxSegment)
	assert.False(t, second.Checked)
	assert.Equal(t, "todo", second.Text)
}
