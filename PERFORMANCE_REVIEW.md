...
**5.3 Idle CPU: true idle event loop — implemented (2026-06-13).** `runGL` (`internal/driver/glfw/loop.go`) now blocks indefinitely in `glfw.WaitEvents` when there are no running animations, no immediately drawable dirty canvases, and no pending `funcQueue` work. Queued main-thread functions, animation start/stop, `Quit`, and GLFW canvas dirty transitions post `glfw.PostEmptyEvent()` so the blocked wait wakes racelessly; a pending-function counter covers the unbounded queue handoff between enqueue and `Out()`. Active work is still paced by the display-derived frame interval via `glfw.WaitEventsTimeout`, with an explicit next-frame deadline so native input events can wake the wait without causing animations to draw faster than the target frame rate. Dirty Wayland surfaces that are not presentable wait for native frame-callback events instead of spinning.
...
4. ✅ **Idle event loop (5.3)** — **implemented** (2026-06-13). `runGL` blocks in `glfw.WaitEvents` when idle, paces active work via the display-derived frame interval, and wakes racelessly on queued funcs, animation start/stop, `Quit`, and canvas dirty transitions; near-zero idle wakeups for background/tray apps.
5. **Space-run cache remainder of 2.3** — next low-risk text-stack item.
6. **Prototype damage tracking (5.1)** behind a build tag, validated by software-painter screenshot diffs, since it dwarfs everything else if it lands.
...
3. ✅ **5.3 (idle loop wakeups)** — implemented; battery/background-CPU win for a long-running utility app sitting mostly idle between ticker updates.
